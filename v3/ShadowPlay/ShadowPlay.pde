...
void draw() {
  ...
  if (gifFrames.size() > 0) {
    ...
    image(snapshot, 0, 0, width, height); // SHOW   
  }

  // if (mirrorMode) {
  //   mirrorSnapshot = opencvCropped.getSnapshot(); // get whatever is currently on opencv, should be processed image video feed
  //   pushMatrix();
...
void keyReleased() {
  if (key == 's' || key == 'S') {
    //snapshot = get();
    snapshot = opencvCropped.getSnapshot(); // get whatever is currently on opencv, should be processed image video feed
    println("snapshot");
  }
  if (key == 'm' || key == 'M') {
